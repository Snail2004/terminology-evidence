...
def _event_schema() -> dict[str, Any]:
    ...
    payloads = [
        ...
        payload(
            ["candidate_id", "sense_id", "semantic_role", "semantic_call_id", "provider_request_id", "retry_index", "provider_id", "model_id", "route", "prompt_sha256", "request_sha256", "response_sha256", "response_physical_sha256", "raw_response_locator", "generation_config", "generation_contract_sha256", "token_accounting_authority_sha256", "provider_role_plan_sha256", "outcome", "latency_ms", "physical_request_count", "network_request_count", "started_at", "completed_at"],
            {"candidate_id": STRING, "sense_id": STRING, "semantic_role": STRING, "semantic_call_id": STRING, "provider_request_id": STRING, "retry_index": NONNEGATIVE, "provider_id": STRING, "model_id": STRING, "route": STRING, "prompt_sha256": SHA256, "request_sha256": SHA256, "response_sha256": SHA256, "response_physical_sha256": SHA256, "raw_response_locator": STRING, "generation_config": {"oneOf": [{"type": "object", "additionalProperties": False, "required": ["temperature", "reasoning"], "properties": {"temperature": {"type": "number", "minimum": 0, "maximum": 2}, "reasoning": STRING}}, {"type": "object", "additionalProperties": False, "required": ["reasoning", "thinking_level"], "properties": {"reasoning": {"const": "minimal"}, "thinking_level": {"const": "minimal"}}}]}, "generation_contract_sha256": SHA256, "token_accounting_authority_sha256": SHA256, "provider_role_plan_sha256": SHA256, "outcome": {"enum": ["SUCCESS", "RETRYABLE_FAILURE", "TERMINAL_FAILURE", "UNKNOWN_PHYSICAL_OUTCOME"]}, "latency_ms": NONNEGATIVE, "physical_request_count": {"type": "integer", "minimum": 1}, "network_request_count": NONNEGATIVE, "started_at": STRING, "completed_at": STRING},
        ),
        payload(["code", "message", "details"], {"code": STRING, "message": STRING, "details": {"type": "object"}}),
    ]
    usage = {"type": "object", "additionalProperties": False, "required": ["input_tokens", "output_tokens", "reasoning_tokens", "total_tokens", "cost", "currency", "cost_status"], "properties": {"input_tokens": NONNEGATIVE, "output_tokens": NONNEGATIVE, "reasoning_tokens": NONNEGATIVE, "total_tokens": NONNEGATIVE, "cost": {"type": ["number", "null"], "minimum": 0}, "currency": {"type": ["string", "null"]}, "cost_status": {"enum": ["LOCAL_FIXTURE_COST_RECORDED", "PROVIDER_BILLED_COST_REPORTED", "TOKEN_ONLY_COST_UNAVAILABLE"]}}}
    return _object(
        "ELiveLedgerEventV1",
        [
            "event_index",
