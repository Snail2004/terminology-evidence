...
ARTIFACT_NAME = "d2l_stage_a_pilot_5_senses_official_v1"
POLICY_ID = "d2l-stage-a-p0b-official-5-sense-v1.0"
...
def _write_report(staging: Path) -> None:
    text = """# D2L Stage A Official 5-Sense Pilot V1
...
def build_official_pilot(
    *,
    ...
    try:
        ...
        write_jsonl(staging / "materialized_input" / "term_senses_5.jsonl", selected_senses)
        write_jsonl(
            staging / "materialized_input" / "candidate_instances_15.jsonl",
            selected_candidates,
        )
        write_jsonl(
            staging / "materialized_input" / "contexts_29.jsonl", selected_contexts
        )
        write_jsonl(
            staging / "materialized_input" / "review_decisions_5.jsonl",
            [row for row in data["decisions"] if row["sense_id"] in selected_ids],
        )
        write_jsonl(
            staging / "materialized_input" / "review_provenance_8.jsonl",
        ...
        for sense in selected_senses:
            ...
            write_json(
                staging / "review_bindings_5" / f"{sense['sense_id']}.json", binding
            )
        ...
        for sense in selected_senses:
            ...
            binding_path = f"review_bindings_5/{sense_id}.json"
            source_hashes = {
            ...
            effective_path = f"effective_sense_contracts_5/{sense_id}.json"
            ...
            for candidate in candidates:
                ...
                frozen_path = f"frozen_candidate_contracts_15/{candidate_id}.json"
        ...
        write_json(
            staging / "candidate_index_15.json",
            seal_integrity(
                {
        ...
        write_json(
            staging / "integration_pilot_5_sense_selection_receipt.json",
        ...
        write_json(
            ...
            seal_integrity(
                {
                    "schema_id": "D2LOfficial5SensePilotLineageV1",
                    "schema_version": "1.0.0",
                    "policy_id": POLICY_ID,
        ...
        acceptance = seal_integrity(
            {
                "schema_id": "D2LOfficial5SensePilotAcceptanceGateReportV1",
                "schema_version": "1.0.0",
                "policy_id": POLICY_ID,
                "status": STATUS,
                "checks": {
                    ...
                    "effective_sense_contract_count_5": len(effective_contracts) == 5,
                    "frozen_candidate_contract_count_15": len(frozen_contracts) == 15,
                    "constraint_evidence_package_count_15": len(constraints) == 15,
        ...
        manifest = {
            "schema_id": "D2LOfficial5SensePilotManifestV1",
            "schema_version": "1.0.0",
            ...
            "status": STATUS,
            "counts": {
                "effective_sense_contract": 5,
                "frozen_candidate_contract": 15,
                "constraint_evidence_package": 15,
                "candidate": 15,
                "selected_sense": 5,
                "stage_b_row": 45,
                "stage_b_eligible": 33,
