...
try:
    ...
    from .stage_b_review_result import STAGE_B_ALLOWED_LABELS
    ...
    from stage_b_review_result import STAGE_B_ALLOWED_LABELS  # type: ignore
...
class ValidatedStageBAdjudication:
    path: Path
    sha256: str
...
def validate_completed_stage_b_adjudication(
    canonical_input_path: Path,
    completed_path: Path,
) -> tuple[ValidatedStageBAdjudication | None, list[str]]:
    errors: list[str] = []
    try:
    ...
    if len(source_cases) != 19 or len(result_cases) != len(source_cases):
        errors.append(f"{prefix}: expected exactly 19 unchanged cases")
    cases_by_candidate: dict[str, dict[str, Any]] = {}
    label_counts = {label: 0 for label in STAGE_B_ALLOWED_LABELS}
    ...
    if len(cases_by_candidate) != 19:
        errors.append(f"{prefix}: adjudication coverage is not 19/19")
    if errors:
        return None, errors
    return (
        ValidatedStageBAdjudication(
            path=completed_path,
