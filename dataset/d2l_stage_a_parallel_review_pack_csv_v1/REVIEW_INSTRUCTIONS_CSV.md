# Parallel review instructions for CSV

Review the five English term-sense cases using only the supplied corpus
contexts. `sense_review_cases.csv` contains case-level information and
`sense_review_contexts.csv` contains the evidence rows. `SENSE_CASEBOOK.md`
presents the same evidence in a readable form.
...
Return the completed assigned CSV as an attached file. Do not paste its contents
into the chat and do not add commentary rows or columns.
