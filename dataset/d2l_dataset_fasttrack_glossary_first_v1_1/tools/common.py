...
def build_file_inventory(root: Path, excluded: set[str] | None = None) -> dict[str, dict[str, Any]]:
...
def write_checksums(root: Path, path: Path) -> None:
    lines = []
    for relative, metadata in build_file_inventory(root, {path.relative_to(root).as_posix()}).items():
        lines.append(f"{metadata['sha256']} *{relative}")
