# D2L Dataset Fast-Track: Glossary First V1.1 Repair

This companion artifact implements `dataset-fasttrack-glossary-first-v1.1`
as a repair companion to the immutable V1 artifact.
...
The current blind pack follows the post-review rule: all three available
development R0 cases, two deterministic clear controls, three registered
disagreements, and five ambiguous/polysemous cases. The earlier blind pack and
result remain under `historical_*` names and are not reused as current results.
...
python tools/build_fasttrack_artifact.py `
  ...
  --output release\d2l_dataset_fasttrack_glossary_first_v1_1 `
  --created-at 2026-07-29T00:00:00Z

python tools/validate_fasttrack_artifact.py `
  --artifact-root release\d2l_dataset_fasttrack_glossary_first_v1_1
```
