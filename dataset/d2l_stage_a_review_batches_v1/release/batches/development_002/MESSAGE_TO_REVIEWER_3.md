You are reviewer slot 3 for batch `development_002` containing 10 English term-sense cases.

Read `REVIEW_INSTRUCTIONS_CSV.md`, `SENSE_CASEBOOK.md`,
`sense_review_cases.csv`, and `sense_review_contexts.csv`. Complete only
`ai_3.csv`. Preserve every immutable ID and hash exactly. Evaluate the
...
Return one attached file named exactly `ai_3.csv`. Do not paste CSV text
