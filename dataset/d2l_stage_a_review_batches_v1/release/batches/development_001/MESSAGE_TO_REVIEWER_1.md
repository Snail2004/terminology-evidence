You are reviewer slot 1 for batch `development_001` containing 10 English term-sense cases.

Read `REVIEW_INSTRUCTIONS_CSV.md`, `SENSE_CASEBOOK.md`,
`sense_review_cases.csv`, and `sense_review_contexts.csv`. Complete only
`ai_1.csv`. Preserve every immutable ID and hash exactly. Evaluate the
...
Return one attached file named exactly `ai_1.csv`. Do not paste CSV text
