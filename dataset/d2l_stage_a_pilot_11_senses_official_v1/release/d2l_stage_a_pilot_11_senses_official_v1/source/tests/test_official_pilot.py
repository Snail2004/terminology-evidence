...
from tools.validate_official_pilot import (  # noqa: E402
    _validate_selection_and_gate,
    validate_artifact,
    validate_zip,
)
...
class OfficialPilotTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.output = cls.base / "first" / "d2l_stage_a_pilot_11_senses_official_v1"
        ...
        cls.zip_path = cls.output.parent / (
            "d2l_stage_a_pilot_11_senses_official_v1_reviewer_handoff.zip"
        )
    ...
    def test_release_gate_and_counts(self) -> None:
        ...
        self.assertEqual(manifest["counts"]["effective_sense_contract"], 11)
        self.assertEqual(manifest["counts"]["frozen_candidate_contract"], 33)
        self.assertEqual(manifest["counts"]["constraint_evidence_package"], 33)
    ...
    def test_stage_b_is_33_eligible_12_blocked_with_zero_gold(self) -> None:
        ...
        with (self.output / "stage_b_eligible_33.csv").open(
            encoding="utf-8-sig", newline=""
        ) as handle:
            eligible = list(csv.DictReader(handle))
        with (self.output / "stage_b_blocked_12.csv").open(
            encoding="utf-8-sig", newline=""
        ) as handle:
            blocked = list(csv.DictReader(handle))
        self.assertEqual(len(eligible), 33)
        self.assertEqual(len(blocked), 12)
        self.assertTrue(all(row["stage_b_eligibility"] == "ELIGIBLE" for row in eligible))
        self.assertTrue(
            all(row["stage_b_eligibility"] == "BLOCKED_BY_STAGE_A" for row in blocked)
        )

    def test_exact_eleven_selection_and_role_specific_evidence(self) -> None:
        selection = strict_json_object(
            self.output / "integration_pilot_11_sense_selection_receipt.json"
        )
        self.assertEqual(
        ...
        bindings = [
            ...
            for path in (self.output / "review_bindings_11").glob("*.json")
        ]
    ...
    def test_stage_b_eligible_must_match_official_candidate_index(self) -> None:
        tampered = self.base / "tampered-stage-b"
        shutil.copytree(self.output, tampered)
        path = tampered / "stage_b_eligible_33.csv"
        with path.open(encoding="utf-8-sig", newline="") as handle:
            rows = list(csv.DictReader(handle))
            headers = list(rows[0])
        rows[0]["candidate_id"] = "candidate_not_in_official_index"
        with path.open("w", encoding="utf-8-sig", newline="") as handle:
            writer = csv.DictWriter(handle, fieldnames=headers)
            writer.writeheader()
            writer.writerows(rows)
        errors: list[str] = []
        _validate_selection_and_gate(tampered, errors)
        self.assertIn(
            "Stage B eligible candidates differ from the official candidate index",
            errors,
        )

    def test_deterministic_rebuild(self) -> None:
        second = self.base / "second" / "d2l_stage_a_pilot_11_senses_official_v1"
        second.parent.mkdir()
