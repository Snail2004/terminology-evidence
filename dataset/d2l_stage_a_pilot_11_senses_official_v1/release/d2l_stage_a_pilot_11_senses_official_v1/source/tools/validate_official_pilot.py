...
EXPECTED_SENSE_IDS = {
    ...
    "d2lce_18c3da2d5bdd6d05a83982fa",
    ...
    "d2lce_ce222dd206341f61f2986f7a",
    "d2lce_d7976d0b101e65c34c011871",
}
EXPECTED_TERMS = {
    "null hypothesis",
...
def _validate_stage_b(
    ...
    try:
        ...
        eligible_rows = read_csv(root / "stage_b_eligible_33.csv")
        blocked_rows = read_csv(root / "stage_b_blocked_12.csv")
    ...
    if len(eligible_rows) != 33 or any(
        row.get("stage_b_eligibility") != "ELIGIBLE" for row in eligible_rows
    ):
        errors.append("Stage B eligible handoff must contain exactly 33 eligible rows")
    if len(blocked_rows) != 12 or any(
        row.get("stage_b_eligibility") != "BLOCKED_BY_STAGE_A" for row in blocked_rows
    ):
        errors.append("Stage B blocked ledger must contain exactly 12 blocked rows")
    expected_eligible = {
        row.get("candidate_id")
        for row in rows
        if row.get("stage_b_eligibility") == "ELIGIBLE"
    }
    expected_blocked = {
        row.get("candidate_id")
        for row in rows
        if row.get("stage_b_eligibility") == "BLOCKED_BY_STAGE_A"
    }
    if {row.get("candidate_id") for row in eligible_rows} != expected_eligible:
        errors.append("Stage B eligible handoff is not the exact eligible projection")
    if {row.get("candidate_id") for row in blocked_rows} != expected_blocked:
        errors.append("Stage B blocked ledger is not the exact blocked projection")
...
def _validate_contracts(root: Path, contracts_root: Path, errors: list[str]) -> None:
    ...
    effective_paths = sorted((root / "effective_sense_contracts_11").glob("*.json"))
    frozen_paths = sorted((root / "frozen_candidate_contracts_33").glob("*.json"))
    constraint_paths = sorted((root / "constraint_evidence_packages_33").glob("*.json"))
    if (len(effective_paths), len(frozen_paths), len(constraint_paths)) != (11, 33, 33):
        errors.append("official contract file counts must be exactly 11/33/33")
        return
    ...
    if len(effective_by_hash) != 11 or len(frozen_by_id) != 33 or len(constraints_by_id) != 33:
        errors.append("contract identities are not unique")
    materialized_candidates = {
        row["candidate_instance_id"]: row
        for row in strict_jsonl(root / "materialized_input" / "candidate_instances_33.jsonl")
    }
    review_bindings = {
        path.stem: strict_json_object(path)
        for path in (root / "review_bindings_11").glob("*.json")
    }
    if len(review_bindings) != 11:
        errors.append("review binding count must be 11")
...
def _validate_selection_and_gate(root: Path, errors: list[str]) -> None:
    try:
        selection = strict_json_object(root / "integration_pilot_11_sense_selection_receipt.json")
        gate = strict_json_object(root / "acceptance_gate_report.json")
        index = strict_json_object(root / "candidate_index_33.json")
    except (OSError, UnicodeError, ValueError, json.JSONDecodeError) as exc:
    ...
    if {row.get("sense_id") for row in records if isinstance(row, Mapping)} != EXPECTED_SENSE_IDS:
        errors.append("selection receipt sense IDs differ from exact eleven")
    if {row.get("source_term") for row in records if isinstance(row, Mapping)} != EXPECTED_TERMS:
        errors.append("selection receipt terms differ from exact eleven")
    if Counter(row.get("selection_group") for row in records) != Counter(
        {
            "CLEAR_LOW_RISK": 4,
            "AMBIGUOUS_POLYSEMOUS": 5,
            "GATE_ADJUDICATION_RISK": 1,
            "QUALIFIED_SCOPE": 1,
        }
    ):
        errors.append("selection receipt distribution is not the reviewed 11-sense shape")
    ...
    if gate.get("official_contract_counts") != {
        ...
        errors.append("acceptance gate contract counts differ from 11/33/33")
    ...
    if index.get("candidate_count") != 33 or len(index.get("entries", [])) != 33:
        errors.append("candidate index count must be 33")
    try:
        eligible_candidate_ids = {
            row["candidate_id"] for row in read_csv(root / "stage_b_eligible_33.csv")
        }
    except (OSError, UnicodeError, csv.Error, ValueError, KeyError) as exc:
        errors.append(f"Stage B eligible/index join: {exc}")
    else:
        ...
        if eligible_candidate_ids != indexed_candidate_ids:
            errors.append("Stage B eligible candidates differ from the official candidate index")
