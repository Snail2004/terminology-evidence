...
def write_checksums(root: Path, path: Path, *, excluded: set[str] | None = None) -> None:
    skip = (excluded or set()) | {path.relative_to(root).as_posix()}
    lines = [
        f"{binding['sha256']}  {relative}"
        for relative, binding in file_bindings(root, excluded=skip).items()
    ]
    write_text(path, "\n".join(lines))
