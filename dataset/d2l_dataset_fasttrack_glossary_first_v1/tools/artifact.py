...
ARTIFACT_NAME = "d2l_dataset_fasttrack_glossary_first_v1"
...
POLICY_TEXT = """# Dataset Fast-Track Policy V1.0
...
def verify_glossary_repository(repository: Path) -> dict[str, str]:
...
def _blind_case_refs(blind_root: Path) -> dict[str, str]:
    return {
        row["sense_id"]: f"stage_a_blind_audit_results.csv#sense_id={row['sense_id']}"
...
def _write_blind_results(output: Path, blind_root: Path) -> None:
    rows = []
...
def build_fasttrack_artifact(
    ...
    (output / "dataset_fasttrack_policy_v1_0.md").write_text(POLICY_TEXT, encoding="utf-8", newline="\n")
    ...
    source_report["blind_audit_status"] = blind_summary["artifact_status"]
    source_report["blind_case_count"] = blind_summary["blind_case_count"]
    source_report["blind_adjudication_case_count"] = blind_summary["adjudication_case_count"]
    ...
    shutil.copyfile(blind_pack_zip, output / "stage_a_blind_audit_pack.zip")
    shutil.copyfile(blind_result_zip, output / "stage_a_blind_audit_result.zip")
    _write_blind_results(output, blind_root)
    ...
    acceptance = {
        ...
        "policy_id": POLICY_ID,
        "status": "BLOCKED_BEFORE_REAL_CE_PILOT",
        "checks": {
            ...
            "blind_audit_complete": blind_summary["artifact_status"] == "PASS",
    ...
    manifest = {
        "schema_id": "D2LDatasetFastTrackGlossaryFirstManifestV1",
        ...
        "counts": {
            ...
            "blind_case": blind_summary["blind_case_count"],
